-- Verify and Fix Enhanced Blog Notification Setup
-- Run this script to check and fix the database setup

-- Step 1: Check current state of customer_notifications table
SELECT 
    'Database Setup Check' as section,
    column_name,
    data_type,
    is_nullable
...
FROM pg_proc 
WHERE proname = 'handle_blog_comment_reply';

-- Step 4: Add missing columns (this is safe to run even if columns exist)
...
-- Step 5: Recreate trigger function (safe to run if it exists)
...
-- Step 6: Drop and recreate trigger (safe to run)
...
SELECT 
    ...
    'All required columns and triggers have been created' as message,
    ...
    (SELECT COUNT(*) FROM pg_trigger WHERE tblname = 'blog_comments' AND evtname = 'trg_blog_comment_reply_notification') as trigger_created;